...
class UptimeKumaServer {
    ...
    async getMonitorJSONList(userID, monitorID = null) {
        ...
        let sql = "";
        // Si no hay condiciones, no agregues WHERE
        if (user.user_type !== "admin") {
            sql = query != null ? ` ${query} ORDER BY weight DESC, name` : "ORDER BY weight DESC, name";
        }

        let monitorList = null;

        if (user.user_type === "admin") {
            monitorList = await R.findAll("monitor");
        } else {
            monitorList = await R.find("monitor", `${sql}`, queryParams);
        }
